"""
统一的大模型调用封装：
- 优先使用当前渠道内置的 Provider（环境变量控制）
- 如果未配置 Provider，则回退到 HTTP 接口（Ark/OpenAI 兼容）
"""
...
API_CONFIG = config["HotList.API"]


def _load_api_credentials() -> Tuple[str, str, str]:
    base_url = API_CONFIG.get("OPENAI_BASE_URL", "https://ark.cn-beijing.volces.com/api/v3").strip()
    if not base_url:
    ...
    if not api_key:
        raise ValueError(
            "请在 config/Identification_Method-Hot_list_word_Dataset-llm_filter-config.ini 的 [HotList.API] 中配置 OPENAI_API_KEY 或 ARK_API_KEY"
        )
...
def call_model(system_prompt: str, user_prompt: str, provider: Optional["LLMProvider"] = None) -> Tuple[Optional[str], Optional[str], Optional[int]]:
    if provider is None and from_environment is not None:
        try:
            ...
            logger.info("[LLMProvider] Using provider from environment in Hot_list_word_Dataset.call_model().")
    ...
    if provider is not None:
        try:
            ...
            content = provider.generate(full_prompt, temperature=0.7)
            logger.info("Provider 返回耗时 %.3fs", time.time() - start_time)
            content = content or ""
            ...
            if 0 <= start_idx < end_idx:
                thinking_part = content[start_idx + len("<think>") : end_idx]
                answer_part = content[end_idx + len("</think>") :].strip()
