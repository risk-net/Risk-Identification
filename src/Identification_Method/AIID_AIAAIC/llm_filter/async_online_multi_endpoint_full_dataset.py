...
CONFIG_PATH = BASE_DIR / "config/Identification_Method-AIID_AIAAIC-llm_filter-config.ini"
CONFIG = configparser.ConfigParser()
if not CONFIG.read(CONFIG_PATH, encoding="utf-8"):
    raise FileNotFoundError(f"无法读取配置文件: {CONFIG_PATH}")

API_CONFIG = CONFIG["AIID_AIAAIC.API"]
ONLINE_CONFIG = CONFIG["AIID_AIAAIC.AsyncOnline"]
...
def _get_int(section: configparser.SectionProxy, option: str, default: int) -> int:
    try:
...
def _get_bool(section: configparser.SectionProxy, option: str, default: bool) -> bool:
    raw = section.get(option, fallback=str(default)).strip().lower()
    if not raw:
        return default
    return raw in {"1", "true", "yes", "on"}


INPUT_JSONL = _resolve_path(ONLINE_CONFIG.get("INPUT_JSONL", "data/standard_cases.jsonl"))
if not INPUT_JSONL.exists():
    raise FileNotFoundError(f"输入 JSONL 文件不存在: {INPUT_JSONL}")

OUTPUT_ROOT = _resolve_path(ONLINE_CONFIG.get("OUTPUT_DIR", "outputs/AIID_AIAAIC/llm_filter"))
OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
MAX_FILES = _get_int(ONLINE_CONFIG, "MAX_FILES", 0)
THREAD_COUNT = _get_int(ONLINE_CONFIG, "THREAD_COUNT", 1)
PER_THREAD_WORKERS = _get_int(ONLINE_CONFIG, "PER_THREAD_WORKERS", 100)
BATCH_SIZE = _get_int(ONLINE_CONFIG, "BATCH_SIZE", 5)
CLIENT_TIMEOUT = _get_int(ONLINE_CONFIG, "CLIENT_TIMEOUT_SECONDS", 3500 * 24 * 10)
CONTENT_LIMIT = _get_int(ONLINE_CONFIG, "CONTENT_LIMIT", 10000)
PROMPT_PATH = _resolve_path(
    ONLINE_CONFIG.get("PROMPT_PATH", "prompt/Identification_Method-AIID_AIAAIC-llm_filter-prompt.md")
)
LOG_DIR = _resolve_path(ONLINE_CONFIG.get("LOG_DIR", "logs/AIID_AIAAIC/llm_filter"))
LOG_DIR.mkdir(parents=True, exist_ok=True)
RUN_TS = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
LOG_PATH = LOG_DIR / f"async_online_multi_model_aiid_aiaaic_{RUN_TS}.log"

logger = logging.getLogger("async_online_multi_model")
...
def _load_models() -> List[str]:
    """
    多模型开关与读取策略（在线推理）：
    - 若 ENABLE_MULTI_MODELS=false 则禁用多模型，仅使用单个模型
    - 若存在 ARK_ONLINE_MODEL_NAMES（逗号分隔）则优先使用其全部值
    - 否则读取 ARK_ONLINE_MODEL_NAME（主）+ ARK_ONLINE_MODEL_NAME_2 + ARK_ONLINE_MODEL_NAME_3 组成列表
    - 如果环境变量未设置，尝试使用 ARK_MODEL_NAME 作为默认值
    - 如果都未设置，抛出异常提示用户设置
    """
    ...
    if disable_multi:
        ...
        if not primary or not primary.strip():
            raise RuntimeError(
                ...
                "请编辑 config/Identification_Method-AIID_AIAAIC-llm_filter-config.ini"
            )
    ...
    out: List[str] = []
    for m in [primary, model2, model3]:
        if m and m.strip():
            out.append(m.strip())
...
def _split_files_round_robin(items: List[Any], k: int) -> List[List[Any]]:
    """将列表按轮询方式分配到 k 个桶中"""
    if k <= 1:
        return [items]
...
def _load_article_from_jsonl(jsonl_entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    从 JSONL 条目加载文章数据
    
    Args:
        jsonl_entry: JSONL 文件中的一行解析后的字典
    
    Returns:
        包含 title, content, release_date, from_database 的字典，如果解析失败则返回 None
    """
    try:
        # 安全地获取字段值，处理 None 的情况
        title = (jsonl_entry.get("title") or "").strip() if jsonl_entry.get("title") else ""
        text = (jsonl_entry.get("text") or "").strip() if jsonl_entry.get("text") else ""
        release_date_raw = jsonl_entry.get("release_date")
        release_date = (release_date_raw or "").strip() if release_date_raw else ""
        from_database_raw = jsonl_entry.get("from_database")
        from_database = (from_database_raw or "").strip().lower() if from_database_raw else ""
        case_id = jsonl_entry.get("id") or jsonl_entry.get("original_id")
        
        # 验证必要字段
        if not title and not text:
            logger.warning(f"[collect] JSONL 条目缺少 title 和 text: {case_id}")
            return None
        
        # 如果没有 release_date，尝试从其他字段提取或使用默认值
        if not release_date:
            release_date = datetime.now().strftime("%Y-%m-%d")
            logger.debug(f"[collect] 使用默认发布时间: {release_date} for case_id={case_id}")
        else:
            # 解析 ISO 格式日期，提取 YYYY-MM-DD 部分
            try:
        ...
        # 如果 title 为空但 text 不为空，使用 text 的前100个字符作为 title
        if not title and text:
            title = text[:100].strip()
        
        # 安全地获取 case_link
        case_link_raw = jsonl_entry.get("case_link")
        case_link = case_link_raw if case_link_raw else ""
        
        return {
            "title": title,
            "content": text[:CONTENT_LIMIT] if text else title,  # 限制内容长度
            "release_date": release_date,
            "from_database": from_database,
            "case_id": str(case_id) if case_id else None,
            "case_link": case_link,
            "original_data": jsonl_entry  # 保留原始数据以便后续使用
        }
    except Exception as exc:
        logger.error(f"[collect] 解析 JSONL 条目失败: {exc}", exc_info=True)
        return None
...
def _collect_requests(
    ...
    """
    收集请求，支持传入 JSONL 条目列表（用于多模型场景）
    
    Args:
        model_name: 在线推理模型名称
        system_prompt: 系统提示词
        jsonl_entries: JSONL 条目列表
        start_task_id: 起始任务ID
        processed_index: 已处理条目的索引（case_id 集合），如果为None则不跳过任何条目
    ...
    def _flush_batch(batch: List[Dict[str, Any]], entry_ids: List[str], tid: int) -> None:
        if not batch or not entry_ids:
            return
        if len(batch) != len(entry_ids):
            logger.warning(
                f"[collect] 批次条目数量不一致: texts={len(batch)} entry_ids={len(entry_ids)} (task={tid})"
            )
            return
        try:
            ...
            requests.append(
                {
                    ...
                    "model": model_name,
                    "entry_ids": entry_ids,  # 保存条目ID用于后续保存
                    "messages": [
    ...
    logger.info(f"[collect] 开始处理 {total_entries} 个条目（model={model_name}）...")
    
    for idx, entry in enumerate(jsonl_entries, start=1):
        try:
            # 检查条目是否已处理
            case_id = str(entry.get("id") or entry.get("original_id", ""))
            if case_id and case_id in processed_index:
                skipped_entries += 1
                continue
            
            article = _load_article_from_jsonl(entry)
            if not article:
                failed_entries += 1
                continue
            ...
            current_entry_ids.append(case_id)
            processed_entries += 1
            
            if len(current_batch) >= BATCH_SIZE:
                _flush_batch(current_batch, current_entry_ids, task_id)
                current_batch = []
                current_entry_ids = []
                task_id += 1
            ...
            if current_time - last_log_time >= log_interval or idx == total_entries:
                progress_pct = (idx / total_entries) * 100
                skip_info = f" | 跳过: {skipped_entries}" if skipped_entries > 0 else ""
                logger.info(
                    f"[collect] 进度: {idx}/{total_entries} ({progress_pct:.1f}%) | "
                    f"已处理: {processed_entries} | 失败: {failed_entries}{skip_info} | 已生成批次: {len(requests)}"
                )
                last_log_time = current_time
                
        except Exception as exc:
            failed_entries += 1
            logger.error(f"[collect] 处理条目失败 {entry.get('id', 'unknown')}: {exc}", exc_info=True)
            continue
    ...
    skip_info = f" | 跳过: {skipped_entries}" if skipped_entries > 0 else ""
    logger.info(
        f"[collect] 完成处理 {total_entries} 个条目 | "
        f"成功: {processed_entries} | 失败: {failed_entries}{skip_info} | 生成批次: {len(requests)}"
    )
...
def _build_processed_index(output_root: Path) -> set:
    """
    构建已处理条目的索引（case_id 集合）
    
    Args:
        output_root: 输出根目录
    
    Returns:
        set: 已处理条目的 case_id 集合
    """
    processed_ids: set = set()
    classifications = ["AIrisk_relevant_event", "AIrisk_relevant_discussion", "AIrisk_Irrelevant"]
    databases = ["aiid", "aiaaic"]  # 支持的数据库类型
    
    logger.info("[collect] 开始扫描输出目录，构建已处理条目索引...")
    ...
    for database in databases:
        for classification in classifications:
            classification_dir = output_root / database / classification
            if not classification_dir.exists():
                continue
            
            # 扫描该分类目录下的所有结果文件
            for result_file in classification_dir.glob("*_result.json"):
                # 提取 case_id（从文件名或文件内容中提取）
                try:
                    with result_file.open("r", encoding="utf-8") as f:
                        result_data = json.load(f)
                        case_id = result_data.get("case_id") or result_data.get("id") or result_data.get("original_id")
                        if case_id:
                            processed_ids.add(str(case_id))
                            total_files += 1
                except Exception as exc:
                    logger.debug(f"[collect] 读取结果文件失败 {result_file}: {exc}")
    
    elapsed = time.time() - start_time
    logger.info(
        f"[collect] 索引构建完成，找到 {total_files} 个已处理的结果文件，"
        f"涉及 {len(processed_ids)} 个唯一条目，耗时 {elapsed:.2f} 秒"
    )
...
async def _save_structured_results(
    entry_ids: List[str],
    jsonl_entries_map: Dict[str, Dict[str, Any]],  # case_id -> jsonl_entry
    ...
    from_database / classification_result 组织目录，并补上原文 content 字段。
    """
    if not structured_results or not entry_ids:
        logger.warning(
            f"[Worker {worker_label}] task {task_id} 无法保存："
            f"entry_ids={len(entry_ids) if entry_ids else 0}, "
            f"structured_results={len(structured_results) if structured_results else 0}"
        )
        return

    # 验证长度匹配
    if len(entry_ids) != len(structured_results):
        logger.warning(
            f"[Worker {worker_label}] task {task_id} 条目ID和结果数量不匹配: "
            f"entry_ids={len(entry_ids)}, structured_results={len(structured_results)}"
        )
        # 取较小的长度，避免索引越界
        min_len = min(len(entry_ids), len(structured_results))
    ...
    try:
        for entry_id, result in zip(entry_ids, structured_results):
            if not result:
                skipped_count += 1
                logger.debug(f"[Worker {worker_label}] task {task_id} 跳过空结果: {entry_id}")
                continue

            # 获取原始 JSONL 条目
            jsonl_entry = jsonl_entries_map.get(entry_id)
            if not jsonl_entry:
                logger.warning(f"[Worker {worker_label}] task {task_id} 找不到原始条目: {entry_id}")
                failed_count += 1
                continue

            # 获取数据库来源
            from_database = jsonl_entry.get("from_database", "").strip().lower()
            if not from_database or from_database not in ["aiid", "aiaaic"]:
                logger.warning(f"[Worker {worker_label}] task {task_id} 无效的数据库来源: {from_database}, 使用默认值 'aiid'")
                from_database = "aiid"
            ...
            result_with_content["case_id"] = entry_id
            result_with_content["from_database"] = from_database
            result_with_content["title"] = jsonl_entry.get("title", "")
            result_with_content["content"] = jsonl_entry.get("text", "")
            result_with_content["release_date"] = jsonl_entry.get("release_date", "")
            result_with_content["case_link"] = jsonl_entry.get("case_link", "")
            # 明确保存 id 和 original_id（确保两者都被保存）
            if "id" in jsonl_entry:
                result_with_content["id"] = jsonl_entry["id"]
            if "original_id" in jsonl_entry:
                result_with_content["original_id"] = jsonl_entry["original_id"]
            # 保留其他原始字段
            for key, value in jsonl_entry.items():
                if key not in result_with_content:
                    result_with_content[key] = value

            # 目标目录：OUTPUT_ROOT / from_database / classification
            results_dir = OUTPUT_ROOT / from_database / classification
            results_dir.mkdir(parents=True, exist_ok=True)

            result_file = results_dir / f"{entry_id}_result.json"
        ...
        logger.info(
            ...
            f"成功={saved_count}, 失败={failed_count}, 跳过={skipped_count}, 总计={len(entry_ids)}"
        )
    except Exception as exc:
        logger.error(
...
async def _run_subset(
    ...
    jsonl_entries_map: Dict[str, Dict[str, Any]],
) -> None:
    logger.info(f"[Thread {thread_id}] 初始化AsyncArk客户端（在线推理，model={model_name}），超时={CLIENT_TIMEOUT}秒")
    ...
    for req in requests_subset:
        # 将 jsonl_entries_map 添加到每个请求中
        req["jsonl_entries_map"] = jsonl_entries_map
...
def _thread_entry(
    ...
    logger.info(f"[Thread {thread_id}] 启动（在线推理，model={model_name}），任务数={len(requests_subset)}")

    try:
        asyncio.run(_run_subset(thread_id, requests_subset, api_key, model_name, shared_stats_queue, jsonl_entries_map))
...
async def _worker(
    ...
    while True:
        ...
        logger.info(f"[Worker {worker_label}] 获取到任务 {payload.get('task_id', 'unknown')}，开始调用在线推理API...")
        try:
            ...
            if not entry_ids:
                logger.warning(
                    f"[Worker {worker_label}] task {payload['task_id']} 缺少条目ID信息，无法保存结果"
                )
                ...
                min_len = min(len(entry_ids), len(structured_results))
                entry_ids_to_save = entry_ids[:min_len]
                results_to_save = structured_results[:min_len]

                if len(entry_ids_to_save) != len(results_to_save):
                    logger.error(
                        f"[Worker {worker_label}] task {payload['task_id']} 保存时长度仍不匹配: "
                        f"entry_ids={len(entry_ids_to_save)}, results={len(results_to_save)}"
                    )
                else:
                    # 从全局变量获取 jsonl_entries_map（通过 queue 传递）
                    jsonl_entries_map = payload.get("jsonl_entries_map", {})
                    await _save_structured_results(
                        entry_ids_to_save, jsonl_entries_map, results_to_save, payload["task_id"], worker_label
                    )
...
def main() -> None:
    start = datetime.now()
    logger.info("=" * 60)
    logger.info("开始：异步在线推理处理 AIID/AIAAIC 数据集（支持多模型）")
    logger.info("=" * 60)

    api_key = API_CONFIG.get("ARK_API_KEY", "").strip() or API_CONFIG.get("OPENAI_API_KEY", "").strip()
    if not api_key:
        raise RuntimeError(
            "请在 config/Identification_Method-AIID_AIAAIC-llm_filter-config.ini 中设置 AIID_AIAAIC.API/ARK_API_KEY 或 OPENAI_API_KEY"
        )
    
    models = _load_models()
    k = len(models)
    logger.info(f"启用 {k} 个在线推理模型并行：{models}")
    logger.info(f"⚠️  注意：使用在线推理接口（client.chat.completions.create），而非批量推理接口")
    api_key_display = f"{api_key[:10]}...{api_key[-4:]}" if len(api_key) > 14 else "****"
    logger.info(f"使用 API Key: {api_key_display}")

    # 读取 JSONL 文件
    logger.info(f"开始读取 JSONL 文件: {INPUT_JSONL}")
        ...
        with INPUT_JSONL.open("r", encoding="utf-8") as f:
            for line_num, line in enumerate(f, start=1):
                ...
                try:
                    ...
                    case_id = str(entry.get("id") or entry.get("original_id", ""))
                    if case_id:
                        jsonl_entries_map[case_id] = entry
                    all_entries.append(entry)
                except json.JSONDecodeError as exc:
                    logger.warning(f"[collect] 第 {line_num} 行 JSON 解析失败: {exc}")
                    continue
    except Exception as exc:
        raise RuntimeError(f"读取 JSONL 文件失败 {INPUT_JSONL}: {exc}")
    
    logger.info(f"[collect] 成功读取 {len(all_entries)} 个条目")
    
    # 统计数据库来源
    database_counts = {}
    for entry in all_entries:
        db = entry.get("from_database", "unknown").lower()
        database_counts[db] = database_counts.get(db, 0) + 1
    logger.info(f"[collect] 数据库来源统计: {database_counts}")
    
    # 应用 MAX_FILES 限制（如果设置了）
    if MAX_FILES > 0:
        original_count = len(all_entries)
        all_entries = all_entries[:MAX_FILES]
        logger.info(f"[collect] 限制处理条目数: {MAX_FILES} (原始: {original_count})")
    
    if not all_entries:
        raise RuntimeError(f"JSONL 文件 {INPUT_JSONL} 中没有有效条目。")

    # 将条目分配到多个模型（轮询方式）
    entry_shards = _split_files_round_robin(all_entries, k)
    logger.info(f"将条目分配到 {k} 个在线推理模型:")
    for i, (model, shard) in enumerate(zip(models, entry_shards), start=1):
        logger.info(f"  Model {i} ({model}): {len(shard)} 个条目")

    system_prompt = _load_system_prompt()
    
    # 构建已处理条目索引（用于跳过已处理的条目）
    logger.info("开始构建已处理条目索引...")
    processed_index = _build_processed_index(OUTPUT_ROOT)
    
    # 为每个模型构建请求
    logger.info("开始为每个模型构建请求...")
    per_model_requests: List[Tuple[str, List[Dict[str, Any]]]] = []
    base_task_id = 1
    
    for model_idx, (model, model_entries) in enumerate(zip(models, entry_shards), start=1):
        try:
            logger.info(f"[Model {model_idx} ({model})] 开始构建请求，条目数: {len(model_entries)}")
            model_reqs, next_task_id = _collect_requests(
                model, system_prompt, model_entries, base_task_id, processed_index
            )
            if model_reqs:
                per_model_requests.append((model, model_reqs))
                logger.info(f"[Model {model_idx} ({model})] 成功生成 {len(model_reqs)} 个批次任务")
            else:
                logger.warning(f"[Model {model_idx} ({model})] 未生成任何批次任务")
            base_task_id = next_task_id
        except Exception as exc:
            logger.error(f"[Model {model_idx} ({model})] 构建请求失败: {exc}", exc_info=True)
            continue

    total_requests = sum(len(reqs) for _, reqs in per_model_requests)
    if total_requests == 0:
        raise RuntimeError("未能生成任何批次任务，请检查文件和数据")
    logger.info(f"共生成 {total_requests} 个批次任务（跨 {k} 个在线推理模型）")

    # 为每个 endpoint 的请求分配线程
    ...
    try:
        # 计算总线程数：每个模型使用 THREAD_COUNT 个线程
        total_threads = k * THREAD_COUNT
        logger.info(f"启动线程池，共 {total_threads} 个线程（{k} 个在线推理模型 × {THREAD_COUNT} 线程/模型）")
        logger.info(f"配置：线程数={THREAD_COUNT}/模型, 每线程工作器数={PER_THREAD_WORKERS}, 批次大小={BATCH_SIZE}, 超时={CLIENT_TIMEOUT}秒（单次API调用）")
        
        with ThreadPoolExecutor(max_workers=total_threads) as executor:
            thread_counter = 1
            for model, model_reqs in per_model_requests:
                if not model_reqs:
                    continue
                # 将该模型的请求均分到 THREAD_COUNT 份
                model_chunks = _split_into_chunks(model_reqs, max(THREAD_COUNT, 1))
    ...
    logger.info(f"使用的在线推理模型: {models}")
