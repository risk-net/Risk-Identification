# -*- coding: utf-8 -*-
#这个脚本用于调用豆包API进行文本分类，支持三级分类（事件相关、讨论相关、无关），并且在批量处理时具有鲁棒的错误处理机制。它会记录每条新闻标题与对应分类标签的日志，方便后续分析和调试。请确保在配置文件中正确设置API密钥和基础URL。
...
def _load_api_credentials() -> Tuple[str, str]:
    """从配置文件中读取 API 基础地址与密钥。"""
    base_url = API_CONFIG.get("OPENAI_BASE_URL", "https://ark.cn-beijing.volces.com/api/v3").strip()
    if not base_url:
        base_url = "https://ark.cn-beijing.volces.com/api/v3"

    api_key = API_CONFIG.get("OPENAI_API_KEY", "").strip()
    if not api_key:
        api_key = API_CONFIG.get("ARK_API_KEY", "").strip()

    if not api_key:
        raise ValueError(
            "请在 config/Identification_Method-CommonCrawlNews-llm_filter-config.ini 的 CommonCrawlNews.API 中配置 OPENAI_API_KEY 或 ARK_API_KEY"
        )
