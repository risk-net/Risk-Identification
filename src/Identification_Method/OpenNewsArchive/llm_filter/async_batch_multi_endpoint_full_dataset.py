...
CONFIG_PATH = os.path.join(BASE_DIR, "config/Identification_Method-OpenNewsArchive-llm_filter-config.ini")
config = configparser.ConfigParser()
if not config.read(CONFIG_PATH, encoding="utf-8"):
    raise FileNotFoundError(f"无法读取配置文件: {CONFIG_PATH}")

API_CONFIG = config["OpenNewsArchive.API"]
BATCH_CONFIG = config["OpenNewsArchive.AsyncBatch"]
ONLINE_CONFIG = config["OpenNewsArchive.AsyncOnline"]
...
if RUN_MODE not in {"batch", "online"}:
    raise ValueError(
        f"OpenNewsArchive.API.RUN_MODE 仅支持 'batch' 或 'online'，当前值: {RUN_MODE}"
    )
...
MODE_INPUT_GLOB = "*.txt" if RUN_MODE == "batch" else "*.json"

INPUT_ROOT = _resolve_path(MODE_CONFIG.get("INPUT_ROOT", "outputs/OpenNewsArchive"))
OUTPUT_ROOT = _resolve_path(
    MODE_CONFIG.get(
        "OUTPUT_DIR",
        "outputs/OpenNewsArchive/llm_filter_results/batch" if RUN_MODE == "batch"
        else "outputs/OpenNewsArchive/llm_filter_results/online",
    )
)
OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)

MAX_FILES = _get_int(MODE_CONFIG, "MAX_FILES", 0)
TARGET_YEARS = _parse_int_list(BATCH_CONFIG.get("TARGET_YEARS", "")) if RUN_MODE == "batch" else []
TARGET_MONTHS = _parse_int_list(BATCH_CONFIG.get("TARGET_MONTHS", "")) if RUN_MODE == "batch" else []
THREAD_COUNT = _get_int(MODE_CONFIG, "THREAD_COUNT", 1)
PER_THREAD_WORKERS = _get_int(
    MODE_CONFIG,
    "PER_THREAD_WORKERS",
    100 if RUN_MODE == "batch" else 25,
)
BATCH_SIZE = _get_int(MODE_CONFIG, "BATCH_SIZE", 5)
CLIENT_TIMEOUT = _get_int(MODE_CONFIG, "CLIENT_TIMEOUT_SECONDS", 7200)
CONTENT_LIMIT = _get_int(MODE_CONFIG, "CONTENT_LIMIT", 10000)
PROMPT_PATH = _resolve_path(
    MODE_CONFIG.get(
        "PROMPT_PATH",
        "prompt/Identification_Method-OpenNewsArchive_Datasets-llm_filter-prompt.md",
    )
)
LOG_DIR = _resolve_path(
    MODE_CONFIG.get(
        "LOG_DIR",
        "logs/OpenNewsArchive/llm_filter/batch" if RUN_MODE == "batch"
        else "logs/OpenNewsArchive/llm_filter/online",
    )
)
LOG_DIR.mkdir(parents=True, exist_ok=True)
RUN_TS = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
LOG_PATH = LOG_DIR / f"async_{RUN_MODE}_multi_endpoint_opennews_{RUN_TS}.log"

logger = logging.getLogger(f"async_opennews_{RUN_MODE}_multi_endpoint")
...
def _load_targets() -> List[str]:
    ...
    if RUN_MODE == "batch":
        ...
        if disable_multi:
            ...
            if not primary:
                raise RuntimeError(
                    ...
                    "请编辑 config/Identification_Method-OpenNewsArchive-llm_filter-config.ini"
                )
            return [primary]

        ids = API_CONFIG.get("ARK_BATCH_MODEL_IDS", "")
        if ids:
            eps = [x.strip() for x in ids.split(",") if x.strip()]
            if eps:
                return eps
        ...
        out = [e for e in [primary, ep2, ep3] if e]
        if not out:
            raise RuntimeError(
    ...
    out = [m for m in [primary, model2, model3] if m]
    if not out:
        raise RuntimeError(
    ...
    if RUN_MODE == "online":
        try:
            ...
            if not title and not content:
                logger.warning(f"[collect] 文件 {path} 缺少 title 和 content")
                return None
            ...
            logger.error(f"[collect] JSON 解析失败 {path}: {exc}", exc_info=True)
            return None
...
def _build_processed_index(output_root: Path, target_years: Optional[List[int]] = None, target_months: Optional[List[int]] = None) -> set:
    ...
    for classification in classifications:
        ...
        if RUN_MODE == "online":
            for result_file in classification_dir.rglob("*_result.json"):
...
def _load_article_content(file_path: Path) -> Optional[str]:
    ...
    if RUN_MODE == "online":
        try:
            ...
            content = data.get("content", "").strip()
            if not content:
                content = data.get("title", "").strip()
            return content[:CONTENT_LIMIT] if content else None
        except json.JSONDecodeError as exc:
            logger.debug(f"[save] JSON 解析失败 {file_path}: {exc}")
            return None
        except Exception as exc:
    ...
    try:
        for file_path, result in zip(file_paths, structured_results):
            ...
            if RUN_MODE == "batch":
                ...
                results_dir = OUTPUT_ROOT / classification
...
async def _run_subset(
    ...
    logger.info(
        f"[Thread {thread_id}] 初始化AsyncArk客户端（{MODE_TARGET_LABEL}={target_id}，模式={RUN_MODE}），"
        f"超时={CLIENT_TIMEOUT}秒"
    )
...
def _thread_entry(
    ...
    logger.info(
        f"[Thread {thread_id}] 启动（模式={RUN_MODE}, {MODE_TARGET_LABEL}={target_id}），任务数={len(requests_subset)}"
    )

    try:
        asyncio.run(_run_subset(thread_id, requests_subset, api_key, target_id, shared_stats_queue))
...
async def _worker(
    ...
    while True:
        ...
        logger.info(
            f"[Worker {worker_label}] 获取到任务 {payload.get('task_id', 'unknown')}，"
            f"开始调用{MODE_LABEL}API..."
        )
        try:
...
def main() -> None:
    start = datetime.now()
    logger.info("=" * 60)
    logger.info(f"开始：OpenNewsArchive 异步{MODE_LABEL}处理（RUN_MODE={RUN_MODE}）")
    logger.info("=" * 60)

    api_key = API_CONFIG.get("ARK_API_KEY", "").strip() or API_CONFIG.get("OPENAI_API_KEY", "").strip()
    if not api_key:
        raise RuntimeError(
            "请在 config/Identification_Method-OpenNewsArchive-llm_filter-config.ini 的 [OpenNewsArchive.API] 中设置 ARK_API_KEY 或 OPENAI_API_KEY"
        )
    ...
    if RUN_MODE == "batch":
        logger.info("⚠️  注意：使用批量推理接口（client.batch.chat.completions.create）")
    else:
        logger.info("⚠️  注意：使用在线推理接口（client.chat.completions.create）")
    ...
    logger.info(f"开始扫描输入目录: {INPUT_ROOT}")
    if RUN_MODE == "batch" and TARGET_YEARS:
        logger.info(f"仅处理以下年份: {TARGET_YEARS}")
    if RUN_MODE == "batch" and TARGET_MONTHS:
    ...
    if RUN_MODE == "batch":
            ...
            for month_dir in sorted(year_dir.glob("[0-9]*")):
                if TARGET_MONTHS:
                    try:
                        ...
                        if month_num not in TARGET_MONTHS:
                            logger.debug(
                                f"[collect] 跳过月份 {year_dir.name}/{month_dir.name}（不在目标月份列表中）"
                            )
                            continue
                    except ValueError:
        ...
        logger.info(f"[collect] 找到 {len(json_files)} 个 JSON 文件在 {INPUT_ROOT}")
    ...
    logger.info(f"[collect] 总共找到 {len(all_files)} 个 {MODE_INPUT_GLOB} 文件")
    
    if not all_files:
        raise RuntimeError(f"目录 {INPUT_ROOT} 下未找到 {MODE_INPUT_GLOB} 文件。")

    # 将文件分配到多个 target（轮询方式）
    ...
    processed_index = _build_processed_index(
        OUTPUT_ROOT,
        target_years=TARGET_YEARS if RUN_MODE == "batch" and TARGET_YEARS else None,
        target_months=TARGET_MONTHS if RUN_MODE == "batch" and TARGET_MONTHS else None,
    )
    ...
    for target_idx, (target, target_files) in enumerate(zip(targets, file_shards), start=1):
        try:
            logger.info(
                f"[{MODE_TARGET_LABEL} {target_idx} ({target})] 开始构建请求，文件数: {len(target_files)}"
            )
            target_reqs, next_task_id = _collect_requests(
                target, system_prompt, target_files, base_task_id, processed_index
            )
            if target_reqs:
                per_target_requests.append((target, target_reqs))
                logger.info(
                    f"[{MODE_TARGET_LABEL} {target_idx} ({target})] 成功生成 {len(target_reqs)} 个批次任务"
                )
            else:
    ...
    try:
        ...
        logger.info(
            f"启动线程池，共 {total_threads} 个线程（{k} 个{MODE_TARGET_LABEL} × {THREAD_COUNT} 线程/{MODE_TARGET_LABEL}）"
        )
        logger.info(
            f"配置：线程数={THREAD_COUNT}/{MODE_TARGET_LABEL}, 每线程工作器数={PER_THREAD_WORKERS}, "
            f"批次大小={BATCH_SIZE}, 超时={CLIENT_TIMEOUT}秒（单次API调用）"
        )
